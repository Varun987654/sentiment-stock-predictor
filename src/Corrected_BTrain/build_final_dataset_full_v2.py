# src/Corrected_BTrain/build_final_dataset_full_v2.py
...
OUT_FILE = "data/06_Features_corrected/merged_features_split_corrected_v2.csv"
...
for ticker in TICKERS:
    # This reads the overwritten files, which is what we need
...
# --- Part A: Daily pre-open and lagged after-close sentiment ---
# FIX: Includes all 5 metrics (count, mean, std, max, min)
...
# --- Part B: Cumulative intra-day trading sentiment ---
# FIX: Includes all 5 metrics (count, mean, std, max, min)
...
trading_news['cum_sum_sq'] = (trading_news['compound']**2).groupby(trading_news.groupby(['ticker', 'date']).ngroup()).cumsum()
trading_news['trading_compound_mean'] = trading_news['cum_sum'] / trading_news['cum_count']
trading_news['trading_compound_std'] = np.sqrt((trading_news['cum_sum_sq'] / trading_news['cum_count']) - (trading_news['trading_compound_mean']**2)).fillna(0)
trading_news['trading_compound_max'] = trading_news.groupby(['ticker', 'date'])['compound'].cummax()
trading_news['trading_compound_min'] = trading_news.groupby(['ticker', 'date'])['compound'].cummin()
trading_news.rename(columns={'cum_count': 'trading_compound_count'}, inplace=True)

# Use merge_asof to correctly join the latest cumulative sentiment to each bin
df = pd.merge_asof(
    df.sort_values('bin_end_time'),
    trading_news[['publishedAt', 'ticker', 'trading_compound_count', 'trading_compound_mean', 'trading_compound_std', 'trading_compound_max', 'trading_compound_min']],
...
# List all desired feature columns
final_feature_columns = [
    ...
    'count_after_close', 'mean_after_close', 'std_after_close', 'max_after_close', 'min_after_close',
