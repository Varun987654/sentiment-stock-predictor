# src/Corrected_train/Phase4_4_backtest_no_fees.py
...
LGBM_WEIGHT = 0.8
XGB_WEIGHT = 0.2
# --- CHANGE: Set transaction cost to zero for this analysis ---
...
# --- 4. RUN THE BACKTEST (NO FEES) ---
print("\nRunning backtest with ZERO transaction costs...")
...
print("\n== Final Overall Backtest Performance (NO Costs) ==")
print(f"   Sharpe Ratio (Daily, Before Costs): {daily_sharpe:.4f}")
print(f"   Cumulative P&L (Before Costs):      {cumulative_pnl:.4f}")

# --- 6. PERFORMANCE BY TICKER ---
print("\n== Performance Breakdown by Ticker (NO Costs) ==")
...
# --- 7. SAVE RESULTS ---
output_path = os.path.join(OUTPUT_DIR, "no_fees_backtest_results.csv")
df_test[['ticker', 'bin_end_time', TARGET, 'prediction', 'pnl']].to_csv(output_path, index=False)
print(f"\nSaved no-fees backtest results to {output_path}")